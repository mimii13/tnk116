...
##############################################
# Network variables.
id = 472 # Todo: Set your three digits from your LiU-ID here.
host = '130.236.81.13'
port = 8718
...
def sendMeasurements():
    ...
    if response and response.get("message_type") != "ERROR":
        tempThresh = response.get("temperature_threshold", tempThresh)
        humidTresh = response.get("humidity_threshold", humidThresh)
...
def updateThreshold(type,value):
    ...
    return response.get("success", False) if response["message_type"] != "ERROR" else False
...
def sendToServer(request):
    ...
    so.sendall(json_msg.encode('utf-8'))
    ...
    response = json.loads(receive.replace("|", ","))
